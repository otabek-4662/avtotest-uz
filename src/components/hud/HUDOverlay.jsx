...
export const HUDOverlay = ({ stageIndex = 0, speed = 0 }) => {
  ...
  return (
    <div className="absolute inset-0 z-20 pointer-events-none p-6 sm:p-12 overflow-hidden flex flex-col justify-end">
      {/* Bottom HUD Dashboard */}
      <div className="flex flex-col sm:flex-row items-end justify-between w-full gap-6">
        
        {/* LEFT COLUMN: Brand, Status, and SpeedGauge */}
        <div className="flex flex-col items-start gap-4">
          <div className="hud-panel p-5 flex flex-col gap-3 pointer-events-auto max-w-sm">
            ...
            <div className="h-px w-full bg-[var(--panel-border)] opacity-50" />
            
            <div className="flex flex-col">
              <span className="tabular-mono text-xs font-bold text-[var(--signal-blue-glow)]">
                STAGE {currentStage.code} // {currentStage.title}
              </span>
              <span className="text-[11px] text-[var(--ink-fog)] mt-1 leading-relaxed">
                {currentStage.desc}
              </span>
            </div>
          </div>
        ...
        {/* RIGHT COLUMN: Action & Telemetry Hint */}
        <div className="hud-panel p-6 flex flex-col items-start sm:items-end gap-3 pointer-events-auto max-w-md w-full sm:w-auto text-left sm:text-right">
          ...
          <p className="text-xs text-[var(--ink-fog)]">
